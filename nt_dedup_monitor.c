...
int main(int argc, char** argv) {
  ...
  int pop_sid = 0;               // --pop-sid=N: stream-id whose drop counter is used for pops
  ...
  while ((opt = getopt_long(argc, argv, "a:i:s:b:Cov:X", long_opts, NULL)) != -1) {
    switch (opt) {
      ...
      case 1010: pop_sid = atoi(optarg); if (pop_sid < 0) pop_sid = 0; if (pop_sid > 255) pop_sid = 255; break;
      default:
  ...
  while (g_running) {
    ...
    // Pop on every poll using stream drop counter for selected SID
    if (enable_capture) {
      ...
      size_t popn = (size_t)((cur >= prev) ? (cur - prev) : cur);
    ...
    if (since_summary >= summary_period) {
      ...
      // Hardware-assisted pop: use dedup drop delta on egress_port to pop queue head(s)
      if (enable_capture && C.egress_port >= 0 && C.egress_port < 64){
        size_t popn = (size_t)dedup_delta_pkts[C.egress_port];
        if (popn){
          size_t did = q_pop_n(&C.q, popn);
          C.pop_total += did;
        }
